...
**(D) 프록시 외의 경유지도 확인.** 토큰은 WebSocket URL 쿼리(`?token=...`)로 전달되므로, 리버스 프록시뿐 아니라
**CDN 접근 로그, APM/모니터링(요청 URL 캡처), 컨테이너/플랫폼 로그**에도 평문으로 남을 수 있다. 운영 시 권장:
> - CDN을 둔다면 WebSocket 경로(`/yjs` 등)는 **로깅 최소화/캐시 우회**로 설정한다.
> - 모니터링(예: 요청 URL을 태그로 수집하는 도구)에서 쿼리스트링을 **스크럽(scrub)** 하거나 경로만 수집한다.
> - 가능하면 **공간 토큰 TTL**(설정의 ‘공간 토큰 만료(일)’)을 짧게 두어, 로그에 남은 토큰의 유효 기간을 제한한다.
