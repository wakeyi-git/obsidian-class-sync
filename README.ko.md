...
2. **학생 목록**에서 `+ 학생 추가`(또는 `명단 붙여넣기`로 일괄) → 이름·학생 ID 입력 (Mirror DB/폴더는 비우면 자동).
3. 학생 카드의 **초대** 클릭 → 플러그인이 학생 계정/DB/권한을 자동 생성하고 **QR + 초대 코드**를 표시.
4. **연결 테스트**로 모든 학생 DB 접근을 확인, **설정 적용**으로 동기화 시작.

> 교사는 패널의 **‘시작하기’ 탭(온보딩 마법사)** 으로 서버 연결 → 학급 정보 → 학생 등록 → 초대 → 첫 동기화를 순서대로 따라갈 수 있습니다.
...
비우면 원본 이름) 선택·전체 학생에게 배포합니다. **미리보기(dry-run)** 로 학생별 대상·동작을 먼저 확인하고,
실행 후 학생별 결과(작성/건너뜀/실패)와 **실패만 재시도**가 패널에 남습니다. `{{studentName}}` `{{studentId}}`
`{{classId}}` `{{date}}` 변수가 학생별로 치환되고, 기존 파일은 건너뛰기(기본)/덮어쓰기/새 이름 정책으로 처리합니다.
...
공유 폴더 문서를 글자 단위로 동시에 편집합니다(Yjs). 파일 동기화(CouchDB)와 **독립된 별도의 Yjs WebSocket
서버**가 필요합니다. 교사가 설정에 서버 URL과 공간 시크릿을 입력하고 공유 공간을 배포하면 학생에게 자동
전파됩니다. 공유 폴더 문서를 편집 모드로 열면 실시간 세션이 연결되고, 서로의 커서·이름이 표시됩니다. 편집
...
갱신되고, **'공간 토큰 만료(일)'** 로 TTL을 둘 수 있습니다. 교사의 토큰·시크릿은 Obsidian Secret Storage에
...
볼 수 있습니다(학생·노트 라벨 + 위치로 이동). 피드백 문서는 대상 노트가 속한 DB(개인 미러 또는 공유)에 저장되어
기존 동기화로 전파되며, 파일이 아니라 메타데이터이므로 vault에는 쓰이지 않습니다. 공유 폴더 문서와 일반 학생
미러 노트 모두에서 동작합니다.
...
├─ settings/                   # 설정 타입 + 설정 탭(학생 카드 UI) + 검증/명단 파싱
├─ core/
│  ├─ couch/
...
└─ ui/                         # 통합 패널(시작하기·피드백·배포·동기화·관리·로그) · RoleSetupModal · InviteModal · ConfirmModal · ResetModal · BackupModal · StudentBulkImportModal
```
