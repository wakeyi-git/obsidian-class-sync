# Class Sync — Yjs 실시간 서버

> 🌐 **English**: [`README.md`](README.md)

Class Sync의 **선택 기능인 실시간 공동 편집**을 위한 작은 **Yjs WebSocket 서버**입니다. 파일 동기화는 CouchDB에서 돌아가며
이 서버 없이도 완전히 동작합니다. 공유 폴더 노트(및 Excalidraw)의 문자 단위 실시간 편집을 원할 때만 실행하세요.

이 폴더의 파일:
- `server.js` — y-websocket 서버. 두 가지 인증 모드(공간별 HMAC `YJS_SECRET`, 또는 레거시 전역 `YJS_TOKEN`).
- `Dockerfile` / `docker-compose.yml` / `package.json` — 컨테이너 빌드 + 실행(영속 저장은 `./data`, LevelDB).
- `disable-yjs-accesslog.sh` — 리버스 프록시 접근 로그에 `?token=`이 남지 않게 하는 시놀로지 DSM 헬퍼.
...
3. **토큰을 로그에 남기지 마세요.** 토큰은 `?token=` 쿼리로 전달되므로, 리버스 프록시/CDN/모니터링의 쿼리 로깅을
   마스킹/비활성화합니다(시놀로지 DSM은 `disable-yjs-accesslog.sh` 참고). 토큰은 WSS로 전송되어 전송 중에는 노출되지
   않으며, 위험은 평문 토큰이 접근 로그에 쌓이는 것입니다.
...
플러그인은 **독립된 두 엔드포인트**와 통신하며, 둘을 잇는 유일한 주체입니다:
...
- **이 Yjs 서버는 CouchDB에 전혀 접속하지 않습니다.** `ws`/`y-websocket` + HMAC만 사용하고, LevelDB(`./data`)에 로컬로
  영속 저장합니다. 실시간 편집이 CouchDB에 반영되는 것은 *플러그인*이 라이브 문서를 주기적으로 스냅샷해 동기화 계층으로
...
- 진짜 제약은 **도달성**(모든 클라이언트가 인터넷으로 둘 다에 닿을 수 있어야 함 — 원격 학생이 있으면 어느 쪽도 LAN 전용
  주소에 둘 수 없음), **HTTPS/WSS**(Obsidian 모바일은 보안 전송 필수), 그리고 이 서버와 플러그인 설정 간의
  **`YJS_SECRET` 일치**입니다.

## 호스팅 선택지 (시놀로지 대안)

두 서버 모두 Docker가 도는 곳이면 됩니다. 이미 가진 환경 / 관리 부담 정도에 따라 고르세요.

**CouchDB** (표준 소프트웨어 — 플러그인은 HTTPS URL + 관리자 계정만 있으면 됨):
...
**Yjs 실시간 서버** — **이 `server/` 이미지**를 원하는 인프라에 올려 실행하세요:
