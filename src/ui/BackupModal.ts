...
export class ExportModal extends Modal {
	...
	onOpen(): void {
		...
		contentEl.createEl("h3", { text: "설정 내보내기" });
		contentEl.createEl("p", {
			cls: "setting-item-description",
			text: "아래 내용을 복사해 보관하거나 다른 기기에 붙여넣으세요. 보안을 위해 비밀번호·Yjs 토큰·학생 비밀번호는 제외됩니다.",
		});
			...
			.addButton((b) =>
				b
					...
					.onClick(async () => {
						...
						new Notice("Class Sync: 설정을 복사했습니다.");
					}),
			)
			.addButton((b) => b.setButtonText("닫기").onClick(() => this.close()));
	}
...
export class ImportModal extends Modal {
	...
	onOpen(): void {
		const { contentEl } = this;
		contentEl.createEl("h3", { text: "설정 가져오기" });
		contentEl.createEl("p", {
			cls: "setting-item-description",
			text: "내보낸 JSON을 붙여넣으세요. 학생·공유 공간·옵션이 복원됩니다. 비밀번호/토큰은 이 기기에서 다시 입력해야 하며, 가져온 학생은 재초대가 필요합니다.",
		});

		const ta = contentEl.createEl("textarea", { cls: "class-sync-backup-input" });
		ta.rows = 12;
		ta.placeholder = "여기에 설정 JSON을 붙여넣기";
			...
			.addButton((b) =>
				b
					...
					.onClick(async () => {
						...
						if (!v) {
							new Notice("Class Sync: 내용을 붙여넣으세요.");
							return;
						}
						this.close();
