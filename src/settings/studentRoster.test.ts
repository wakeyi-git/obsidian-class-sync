...
import { parseStudentRoster, finalizeRoster } from "./studentRoster";

describe("parseStudentRoster", () => {
	it("쉼표/공백/이름만 형식을 파싱", () => {
		const r = parseStudentRoster("홍길동,hong\n김학생 kim_student\n이영희");
		expect(r).toEqual([
			{ name: "홍길동", id: "hong" },
...
describe("finalizeRoster", () => {
	...
	it("명시 ID는 정규화(소문자/허용문자)", () => {
		const r = finalizeRoster([{ name: "A", id: "Kim Student!" }], []);
		expect(r[0].id).toBe("kim_student");
	});
