...
describe("이중 업로드 방지 (중첩 공유 폴더)", () => {
	...
	it("개인 링크는 childRoots(공유 폴더) 아래 파일을 업로드 대상에서 제외한다", async () => {
		const c = createCore("nestNS", "teacher1");
		core = c;

		// 한 vault 안에 두 링크: 개인(students/alice) + 공유(students/alice/shared, 개인의 하위).
		const personal = buildLink(c.core, {
			localRoot: "students/alice",
			remoteDb: "mirror_alice",
			childRoots: ["students/alice/shared"],
		});
		const shared = buildLink(c.core, {
			localRoot: "students/alice/shared",
			remoteDb: "share_g1",
		});

		// 개인 파일 + 공유 폴더 안 파일.
		c.vault.seed("students/alice/notes/p.md", "personal");
		c.vault.seed("students/alice/shared/group.md", "shared-doc");

		// 직접 업로드 시도: 개인 링크는 공유 파일을 제외해야 한다.
		expect(await personal.uploader.uploadPath("students/alice/shared/group.md")).toBe("skipped-excluded");
		expect(await shared.uploader.uploadPath("students/alice/shared/group.md")).toBe("uploaded");
